...
$mq="SELECT * FROM `questionnaire` JOIN `courses` JOIN `uc_users` WHERE questionnaire.prof_id =".$prof_id." AND questionnaire.course_id =".$course_id." AND courses.id=questionnaire.course_id AND questionnaire.prof_id=uc_users.id ";
$result=mysqli_query($con,$mq);


echo '<div id="main">';
echo '<section class ="head1">';
echo 'Topics taught recently for your Courses';
echo '</section>';
...
while($row = mysqli_fetch_array($result)) {
    ...
    echo $row['Topic'];
    echo "</td>";
    echo "<td>  <form method='POST' action='quesshow.php'>
